...
namespace ES3Types
{
	...
	[ES3PropertiesAttribute("storedItems", "itemGrid", "currentRoomLevelData", "roomData", "position", "currentLevel", "currentHitPoints", "roomType", "isDamageable", "connectedDoors", "currentRotation", "crewInRoom", "isActive", "isPowered", "isPowerRequested", "roomRenderer", "parentShip", "icon", "gridSize", "damageCondition", "workDirection")]
		...
		protected override void ReadComponent<T>(ES3Reader reader, object obj)
		{
			...
			foreach(string propertyName in reader.Properties)
			{
				switch(propertyName)
				{
					...
					break;
					case "roomData":
					instance = (StorageRoomRegular)reader.SetPrivateField("roomData", reader.Read<RoomData>(), instance);
					break;
					case "position":
					...
					break;
					case "roomRenderer":
					instance = (StorageRoomRegular)reader.SetPrivateField("roomRenderer", reader.Read<UnityEngine.SpriteRenderer>(), instance);
					break;
					case "parentShip":
						instance.parentShip = reader.Read<Ship>(ES3UserType_Ship.Instance);
						break;
					case "icon":
					instance = (StorageRoomRegular)reader.SetPrivateField("icon", reader.Read<UnityEngine.SpriteRenderer>(), instance);
					break;
					case "gridSize":
