...
public class DefaultCrewManagerScript : MonoBehaviour
{
    // 승무원 상태 변경 이벤트
    ...
    public static DefaultCrewManagerScript Instance { get; private set; }
    ...
    {
        int targetIndex = effect.targetCrewIndex;
        ...
        {
            CrewMember targetCrew = crew[targetIndex];
            effect.healthChange = 0.0f; // 체력 변경 값 (피해량, 치유량)

            // 효과 유형에 따른 처리
            switch (effect.effectType)
            {
                // 피해 = (기본 공격력 + 장비) * (1 - (상대 방어력) / 100)
                case CrewEffectType.Hit:
                    break;
