...
=======
    ...
    [Serialize] private float repairDelay=0.5f;
    [Serialize] private int repairCoefficient = 5;
    // public IEnumerator RepairRoutine()
    // {
    //     //1. 방 안에 적이 있는지 확인하고
    //     //2. 방이 수리가 필요한 상태인지 확인하고
    //     //3. 방이 수리할 수 있는 상태인지 확인한다.
    //     //4. 방을 수리한다.
    //     // if (!isThereEnemyInCurruntRoom() && currentRoom.NeedsRepair())
    //     // {
    //     //     if(currentRoom.)
    ...
    //----------적 탐지-------------
    public bool isThereEnemyInCurruntRoom()
    {
        List<CrewBase> someone = currentRoom.crewInRoom;
        if (someone.Count == 1)
        {
            return false;
        }

        foreach (CrewBase crew in someone)
        {
            if (crew.isPlayerControlled != isPlayerControlled)
            {
                return true;
            }
        }
