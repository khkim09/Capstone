        ...
        foreach (CrewBase baseCrew in crewList)
        {
            ...
            foreach (CrewEffect effect in crewEffects)
            {
                ...
                if (effect.healthChange != 0)
                    crew.health += effect.healthChange;

                // 스킬 변화
                if (effect.skill != SkillType.None && effect.skillChange != 0)
                {
                    if (crew.skills.ContainsKey(effect.skill))
                        crew.skills[effect.skill] += effect.skillChange;
                    else
                        crew.skills[effect.skill] = effect.skillChange;
                }

                // 상태 이상 적용
                if (effect.statusEffect != CrewStatus.Normal)
                    crew.status = effect.statusEffect;
            }
        }
